...
function createUser(_username, _password, _email, _birthday, sex, lookingFor,
...
{
    let newUser = new UserObject(_username, _password, _email, _birthday, sex, lookingFor,
                    firstName, lastName, _adress, district, profilePic, height,
                    hairColor, eyeColor, smoker, hasAnimals, vegetarian,
                    employed, hasKids, interests, aboutSelf, aboutMatch,
                    prefEyeColor, prefDistrict, prefMaxHeight, prefMinHeight, prefHairColor,
...
let testStorage = JSON.stringify(userDatabase);

console.log(testStorage);

let newTestStorage = JSON.parse(testStorage);

console.log(newTestStorage);
...
function loginAuthenticatedUser(){
    ...
    for(let i = 0; i < users.length; i++){
        ...
        if((users[i].getEmail() == userName || users[i].getUserName() == userName) &&
